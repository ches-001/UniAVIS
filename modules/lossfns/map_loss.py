...
class VectorMapLoss(TrackLoss):
    ...
    def __init__(
        ...
        super(VectorMapLoss, self).__init__(self, cls_lambda, iou_lambda, l1_lambda, angle_lambda=0)
    ...
    def forward(
        ...
        :pred_detections: (N, num_elements, 4 + num_classes)

        target_polylines: (N, num_elements, num_vertices, 2), expected to be long tensor (int64)
            last axis corresponds to x, y
                    
        :target_detections: (N, num_elements, 5) (5 = 4 box data (x, y, w, h) and 1 class label)
        """
        ...
        pred_xywh = pred_detections[:, :, 0:4]
        pred_cls_logits = pred_detections[:, :, 4:]

        target_xywh = target_detections[:, :, 0:4]
        target_cls = target_detections[:, :, 5]
    ...
    def _forward_match(
        ...
        cls_loss = cls_loss[det_loss_mask]
        det_loss_mask[torch.ones_like(target_cls)[:, :, None] & (target_cls == bg_cls_idx)[:, None, :]] = 0
...
class RasterMapLoss(VectorMapLoss):
    def __init__(self, eps: float=1e-5):
        super(RasterMapLoss, self).__init__()
        self.eps = eps
    ...
    def forward(
        ...
        pred_detections (N, num_elements, 4 + k + num_classes) (k = num segmentation proto coefficients)
        ...
        batch_size = pred_detections.shape[0]
        num_elements = pred_detections.shape[1]
        num_seg_coefs = protos.shape[0]

        batch_indexes = torch.arange(batch_size, device=device)[:, None].tile(1, num_elements)

        pred_xywh = pred_detections[:, :, 0:4]
        pred_cls_logits = pred_detections[:, :, 4 + num_seg_coefs:]
        ...
        pred_seg_coefs = pred_detections[:, :, 4:4 + num_seg_coefs]
        pred_seg_coefs =  pred_seg_coefs[batch_indexes, pred_indexes, :][polygen_loss_mask]
        target_polylines = target_polylines[batch_indexes, target_indexes, ...][polygen_loss_mask]

        pred_seg_masks = torch.einsum("nk,khw->nhw", pred_seg_coefs, protos)
