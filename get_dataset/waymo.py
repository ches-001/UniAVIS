...
def process_perception_frame(
    ...
    map_img_path = os.path.join(MAP_IMAGE_LOCAL_PATH, f"{sample_name}_frame_{frame_idx}.png")
    ...
    camera_proj_path = os.path.join(CAMERA_PROJ_LOCAL_PATH, f"{sample_name}_frame_{frame_idx}.npz")
    np.savez(camera_proj_path, camera_proj)

    camera_labels_path = os.path.join(CAMERA_LABELS_LOCAL_PATH, f"{sample_name}_frame_{frame_idx}.json")
    laser_labels_path = os.path.join(LASER_LABELS_LOCAL_PATH, f"{sample_name}_frame_{frame_idx}.json")
    save_json_file(camera_labels, camera_labels_path)
    save_json_file(laser_labels, laser_labels_path)

    data[frame_idx] = {
        "camera_view_paths": camera_view_paths,
        "laser": {
